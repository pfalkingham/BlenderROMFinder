...
def count_range_steps(min_val, max_val, inc):
    """Return the number of unique steps generated by gen_range logic."""
    if inc == 0:
        return 1
...
class ROMProcessor:
    ...
    def _generate_pose_ranges(props):
        ...
        return (
            gen_range(props.rot_x_min, props.rot_x_max, props.rot_x_inc),
            gen_range(props.rot_y_min, props.rot_y_max, props.rot_y_inc),
            gen_range(props.rot_z_min, props.rot_z_max, props.rot_z_inc),
            gen_range(props.trans_x_min, props.trans_x_max, props.trans_x_inc),
            gen_range(props.trans_y_min, props.trans_y_max, props.trans_y_inc),
            gen_range(props.trans_z_min, props.trans_z_max, props.trans_z_inc),
        )
    ...
    def _check_collision(self):
        ...
        Respects the debug "turn off collisions" flag at this level so
        the detector itself stays pure.
        """
        if getattr(self.props, 'debug_mode', False) and getattr(self.props, 'turn_off_collisions', False):
            return False
